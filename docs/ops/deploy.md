...
Single binary (`serpotter-api`) + SQLite. Schema version **13** (`EXPECTED_SCHEMA_VERSION`).

| Probe | Path | Meaning |
| --- | --- | --- |
| Liveness | `GET /live` | process up |
| Readiness | `GET /ready` | DB migrated and schema ≥ 13 |
...
1. `GET /live` → 200
2. `GET /ready` → 200 (schema migrated to ≥ 13)
