//! Wave 3B surface tests: /api/similar (B24), /api/map (B25), batch/question/
//! highlights extract (B26/B27), research backend + citation validation
//! (B17/B31), exa deep search routing (B20/B29), outputSchema (B28).
...
#[tokio::test]
async fn similar_missing_token_401() {
...
#[tokio::test]
async fn similar_missing_url_400() {
    let db = test_db().await;
...
/// Deterministic: no exa key → NoHealthyKey 503 before any network call.
#[tokio::test]
async fn similar_without_exa_key_is_503() {
    let db = test_db().await;
    db.insert_token(TEST_TOKEN, "t").await.unwrap();
    let app = app(state_with(db));
    let res = app
...
#[tokio::test]
async fn map_missing_token_401() {
    let db = test_db().await;
    let app = app(state_with(db));
    let res = app
        .oneshot(
...
/// Deterministic: no firecrawl key → NoHealthyKey 503.
#[tokio::test]
async fn map_without_firecrawl_key_is_503() {
    let db = test_db().await;
    db.insert_token(TEST_TOKEN, "t").await.unwrap();
    let app = app(state_with(db));
    let res = app
